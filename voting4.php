...
// Check if the user has already voted for Escort
$hasVoted = false;
$sql = "SELECT * FROM votes WHERE user_id = ? AND position = 'Escort'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['candidate']) && !$hasVoted) {
    ...
    // Record the vote
    $insertVoteSql = "INSERT INTO votes (user_id, candidate_id, position) VALUES (?, ?, 'Escort')";
    ...
    if ($stmt) {
        ...
        if ($candidateId) {
            ...
            // Redirect to logout.php after successful vote
