...
// Fetch candidates for the 'Muse' position only
$sqlMuse = "SELECT * FROM candidates WHERE position = 'Muse'";
$candidatesResultMuse = $conn->query($sqlMuse);

// Prepare an array to hold candidates data for the 'Muse' position
$candidatesMuse = [];
if ($candidatesResultMuse->num_rows > 0) {
    while ($row = $candidatesResultMuse->fetch_assoc()) {
        $candidatesMuse[] = $row;
    }
}
$conn->close();
