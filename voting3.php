...
$sql = "SELECT * FROM candidates WHERE position = 'Muse'";
...
// Check if the user has already voted for Muse
$hasVoted = false;
$sql = "SELECT * FROM votes WHERE user_id = ? AND position = 'Muse'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['candidate']) && !$hasVoted) {
    ...
    // Record the vote
    $insertVoteSql = "INSERT INTO votes (user_id, candidate_id, position) VALUES (?, ?, 'Muse')";
